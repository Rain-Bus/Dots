...
M.lsp = function(bufnr)
  ...
  map("n", "gR", "<cmd>TroubleToggle lsp_references<cr>", { desc = "Show references", buffer = bufnr })
  map("n", "gi", "<cmd>TroubleToggle lsp_implementations<cr>", { desc = "Show implementations", buffer = bufnr })
  ...
  map("n", "<LEADER>lc", "<cmd>Lspsaga show_cursor_diagnostics<cr>", { desc = "Show cursor diagnostic", buffer = bufnr })
  map("n", "<LEADER>lf", "<cmd>lua vim.lsp.buf.format({ async = true })<cr>", { desc = "Show cursor diagnostic", buffer = bufnr })
end
